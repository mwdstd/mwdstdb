...
from mwdstdb.models import Survey, Station, BHA, Interval, Reference, CiStation
from mwdstdb.models.units import Density, Length, StorageUnitSystem, CalculationUnitSystem
...
async def correction_lv3(db: AsyncIOMotorDatabase, borehole_id: str, filter_visibility: bool = True):
    pipeline = [
        ...
        {'$project' : {
            'runs._id': 1,
            'runs.surveys': 1, 'runs.reference': 1, 'runs.interference_intervals': 1, 'runs.bha': 1, 'runs.mud_weight': 1,
            'runs.correction': 1, 'runs.casing_depth': 1, 'runs.correction_id': 1, 'runs.ci': 1,
